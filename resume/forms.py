...
class MonthYearField(forms.Field):
    def to_python(self, value):
        ...
        try:
            ...
            raise forms.ValidationError("Invalid data format. It should be YYYY-MM")
...
class UserInfoForm(forms.Form):
    ...
    skills = forms.CharField(
        widget=forms.Textarea(
            attrs={"rows": 1, "placeholder": "Enter your skills separated by commas..."}
        )
    )
...
class ExperienceForm(forms.Form):
    ...
    end_date = MonthYearField(
        widget=MonthYearDateInput(format="%Y-%m"), label="End Date", required=False
    )
    current_role = forms.BooleanField(
        label="I am currently working in this role", required=False
    )
