...
from resume.services import agent_loop, agent_tools, job_service
...
class RemeasureTests(TestCase):
    def setUp(self):
        self.user = User.objects.create_user("measurer", password="x")
        self.resume = Resume.objects.create(user=self.user, title="Main", content=CONTENT)
        self.posting = JobPosting.objects.create(user=self.user, title="Backend", description=POSTING)
        self.posting.take_snapshot({"user_info": {"full_name": "Old copy"}}, "faangpath-simple", self.resume)
        self.posting.snapshot_taken_at = timezone.now() - timezone.timedelta(hours=1)
        self.posting.save()

    def measure(self, **kwargs):
        with patch("resume.services.job_match.typesafe_engine.ask", side_effect=fake_ask), \
             patch("resume.services.job_match.send_openai_message", return_value="{}"):
            return job_service.remeasure(self.posting, "en", **kwargs)

    def test_an_unsent_application_measures_the_edited_resume_and_follows_it(self):
        self.resume.save()  # edited after the copy was taken
        result, measured = self.measure()
        self.assertEqual(measured, "resume")
        self.assertEqual(self.posting.snapshot_content["user_info"]["full_name"], "Ada")

    def test_a_sent_application_measures_what_was_sent(self):
        self.posting.set_status("applied")
        self.resume.save()
        _, measured = self.measure()
        self.assertEqual(measured, "stored_copy")
        self.assertEqual(self.posting.snapshot_content["user_info"]["full_name"], "Old copy")

    def test_an_untouched_resume_keeps_the_stored_copy(self):
        self.posting.snapshot_taken_at = timezone.now() + timezone.timedelta(minutes=1)
        _, measured = self.measure()
        self.assertEqual(measured, "stored_copy")

    def test_the_tool_reports_what_it_measured(self):
        self.resume.save()
        with patch("resume.services.job_match.typesafe_engine.ask", side_effect=fake_ask), \
             patch("resume.services.job_match.send_openai_message", return_value="{}"):
            result = agent_tools.get_tool("rescore_job").handler(
                self.user, {"lang": "en"}, job_id=self.posting.id)
        self.assertEqual(result.data["measured"], "the user's current resume")
        self.assertEqual(result.ui[0]["resume_id"], self.resume.id)
